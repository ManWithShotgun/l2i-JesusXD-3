...
import ru.catssoftware.gameserver.mmocore.HeaderInfo;
import ru.catssoftware.gameserver.mmocore.IClientFactory;
import ru.catssoftware.gameserver.mmocore.IMMOExecutor;
import ru.catssoftware.gameserver.mmocore.IPacketHandler;
import ru.catssoftware.gameserver.mmocore.MMOConnection;
import ru.catssoftware.gameserver.mmocore.ReceivablePacket;
import ru.catssoftware.gameserver.mmocore.TCPHeaderHandler;
import ru.catssoftware.gameserver.network.L2GameClient.GameClientState;
import ru.catssoftware.gameserver.network.clientpackets.*;
...
//						_log.info(String.format("Unknown packet %x from "+client,opcode));
		...
		return msg;
			...
			_log.warn("Exception while handle client packet",e);
			return null;
		}
	}
	...
	/**
	 * Проверка кол-ва неизвестных пакетов
	 * По умолчанию пропускаем 10 пакетов
	 */
	private void handleUnknown(GameClientState state, L2GameClient client)
	{
		switch(state)
		{
			case CONNECTED:

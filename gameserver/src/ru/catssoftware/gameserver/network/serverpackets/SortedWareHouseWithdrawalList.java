...
import ru.catssoftware.gameserver.templates.item.L2WarehouseItem;
...
public class SortedWareHouseWithdrawalList extends L2GameServerPacket
{
	...
	private List<L2WarehouseItem> createWeaponList(L2ItemInstance[] _items)
	{
		...
		{
			if (item.isWeapon() || item.getItem().getType2() == L2Item.TYPE2_WEAPON || (item.isEtcItem() && item.getItemType() == L2EtcItemType.ARROW)
	...
	private List<L2WarehouseItem> createAmulettList(L2ItemInstance[] _items)
	{
		...
		for (L2ItemInstance item : _items)
		{
			if (item.isEtcItem() && (item.getEtcItem().getItemType() == L2EtcItemType.SPELLBOOK && item.getItemName().toUpperCase().startsWith("AMULET"))
					|| item.getItem().getType2() == L2Item.TYPE2_MONEY)
			{
				if (_list.size() <= 200)
					_list.add(new L2WarehouseItem(item));
	...
	private List<L2WarehouseItem> createConsumableList(L2ItemInstance[] _items)
	{
		...
		for (L2ItemInstance item : _items)
		{
			if (item.isEtcItem() && (item.getEtcItem().getItemType() == L2EtcItemType.SCROLL || item.getEtcItem().getItemType() == L2EtcItemType.SHOT)
					|| item.getItem().getType2() == L2Item.TYPE2_MONEY)
			{
				if (_list.size() <= 200)
					_list.add(new L2WarehouseItem(item));
	...
	private List<L2WarehouseItem> createOtherList(L2ItemInstance[] _items)
	{
		...
		{
			if (item.isEtcItem()
					&& (item.getEtcItem().getItemType() != L2EtcItemType.MATERIAL && item.getEtcItem().getItemType() != L2EtcItemType.RECEIPE
							&& item.getEtcItem().getItemType() != L2EtcItemType.SPELLBOOK && item.getEtcItem().getItemType() != L2EtcItemType.SCROLL && item
							.getEtcItem().getItemType() != L2EtcItemType.SHOT) || item.getItem().getType2() == L2Item.TYPE2_MONEY)
			{
				if (_list.size() <= 200)
					_list.add(new L2WarehouseItem(item));
				else
	...
	protected final void writeImpl(L2GameClient client,L2PcInstance activeChar)
	{
		...
		for (L2WarehouseItem item : _objects)
		{
			...
			if (item.isAugmented())
			{
				writeD(0x0000FFFF & item.getItemInstance().getAugmentation().getAugmentationId());
				writeD(item.getItemInstance().getAugmentation().getAugmentationId() >> 16);
			}
			else
				writeQ(0x00);
		}

...
{
	private static ScheduledThreadPoolExecutor interestPool = new ScheduledThreadPoolExecutor(Config.INTEREST_MAX_THREAD, new ThreadPoolManager.PriorityThreadFactory("InterestManagerPool", 4));
	static
	{
		interestPool.setKeepAliveTime(1, TimeUnit.SECONDS);
		interestPool.allowCoreThreadTimeOut(true);
	}
	...
	{
		if(Config.PROTECT_COMPRESSION_WRITEDELAY < 1)
			return true;
		long now = System.currentTimeMillis();
		if(now - lastWrite < Config.PROTECT_COMPRESSION_WRITEDELAY)
			return false;
		lastWrite = now;
		return true;
	}
	...
	{
		return  _client != null && _client instanceof L2GameClient;
	}

	@Override
	public String toString()
	{
		return "MMOConnection: selector=" + _selectorThread + "; client=" + getClient() + "; buffersReleased=" + buffersReleased + "; buffersNulled=" + buffersNulled;
	}
}
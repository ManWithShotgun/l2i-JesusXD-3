...
import ru.catssoftware.gameserver.MonsterRace;
import ru.catssoftware.gameserver.ThreadPoolManager;
import ru.catssoftware.gameserver.datatables.ItemTable;
import ru.catssoftware.gameserver.model.L2ItemInstance;
import ru.catssoftware.gameserver.model.actor.knownlist.RaceManagerKnownList;
...
import ru.catssoftware.gameserver.network.serverpackets.MonRaceInfo;
...
public class L2RaceManagerInstance extends L2NpcInstance
{
	...
	public L2RaceManagerInstance(int objectId, L2NpcTemplate template)
	{
		...
		if (_notInitialized)
		{
			...
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_TICKETS_AVAILABLE_FOR_S1_RACE),
                                         0, 10 * MINUTE);
			...
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_TICKETS_STOP_IN_S1_MINUTES),
                                         2 * MINUTE, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_TICKETS_STOP_IN_S1_MINUTES),
                                         3 * MINUTE, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_TICKETS_STOP_IN_S1_MINUTES),
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		if (command.startsWith("BuyTicket"))
		{
			...
			if ((val == 10 && player.getRace(0) == 0) || (val == 20 && player.getRace(0) == 0 && player.getRace(1) == 0)) val = 0;
	...
	public void showBuyTicket(L2PcInstance player, int val)
	{
		...
		else
		{
			...
			L2ItemInstance item = ItemTable.getInstance().createItem("RaceManager", 4443, 1, player, this); 
	...
	class RunEnd implements Runnable
	{
		public void run()
		{
			...
			makeAnnouncement(SystemMessageId.MONSRACE_S1_END);
			_raceNumber++;
